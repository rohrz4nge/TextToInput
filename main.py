...
def write_char_to_stdin(character=None):
    if character:
...
def write_string_to_stdin(text=None, typing_timeout=0.05):
    for ch in text:
        write_char_to_stdin(ch)
        sleep(typing_timeout)
    # writing the new line
    write_char_to_stdin(Key.enter)
...
possible arguments:
    ...
    typing_timeout: the timeout between each keystroke in seconds """
def write_lines_to_stdin(text=None, path_to_file=None, starting_pause=5, pause_between_lines=2, typing_timeout=0.05):
